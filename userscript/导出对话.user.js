// ==UserScript==
// @name         导出对话（Gemini / Grok / ChatGPT）
// @namespace    wwy.export.chat
// @version      3.0
// @description  边滚边采集（四种滚动手段轮换），把对话每一轮抓下来导出 Markdown 和 JSON。纯 DOM 搬运，不经过任何模型。
// @author       王唯伊
...
(function () {
  ...
  console.log('[导出对话 v3] 脚本已加载：' + location.href);

  const exportChat = async function () {
  ...
  const run = function () {
    return exportChat().catch(function (e) {
      console.error('[导出对话] 出错：', e);
      alert('导出失败，请打开控制台看报错信息。');
    });
  };

  if (typeof GM_registerMenuCommand === 'function') {
    GM_registerMenuCommand('⬇︎ 导出这个对话', run);
    console.log('[导出对话 v3] 已注册油猴菜单命令：点浏览器右上角油猴图标即可看到');
  }

  function mountButton() {
    if (!document.body) return;
    if (document.getElementById('wwy-export-btn')) return;
    const btn = document.createElement('button');
    btn.id = 'wwy-export-btn';
    btn.type = 'button';
    ...
    btn.style.cssText = [
      ...
      'background:#b07515', 'color:#fff', 'font-size:13px', 'font-weight:600',
    ...
    btn.onclick = async function () {
      ...
      try {
        ...
        console.error('[导出对话] 出错：', e);
        btn.textContent = '失败，看控制台';
      }
      setTimeout(function () { btn.textContent = '导出对话'; btn.disabled = false; }, 2400);
    };
    document.body.appendChild(btn);
    console.log('[导出对话 v3] 悬浮按钮已挂载（右下角）');
  }
  ...
  else new MutationObserver(function (mut, ob) {
    if (document.body) { ob.disconnect(); start(); }
